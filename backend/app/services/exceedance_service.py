...
查询/筛选/排序/统计口径统一在 ``exceedance_filters`` 模块,
本文件只保留标注动作与工作台统计聚合。
"""
...
from ..domain.metrics import round3
from ..errors import NotFoundError, ValidationError
from ..extensions import db
from ..models import Exceedance, Station
from ..models.base import iso
from .exceedance_filters import (
    apply_exceedance_filters,
    build_exceedance_query,
    parse_exceedance_filters,
)
...
def exceedance_query(args):
    """返回按统一口径排序后的超标记录查询 (兼容既有内部调用方)."""
    query, _ = build_exceedance_query(args)
    return query
...
def summary(args):
    """Dashboard counters for the annotation work bench (与列表同筛选)."""
    filters = parse_exceedance_filters(args)
    base = apply_exceedance_filters(db.session.query(Exceedance), filters)
    ...
    top_pollutants = [
        {"key": pollutant, "count": int(count), "avg_ratio": round3(avg_ratio) if avg_ratio is not None else 0.0}
    ...
    return {
        ...
        "max_ratio": round3(totals[1]) if totals[1] is not None else 0.0,
        "avg_ratio": round3(totals[2]) if totals[2] is not None else 0.0,
